...
def multires_block(x, filter_count, alpha=1.67, name=None):
    with tf.name_scope(name) as scope:
        ...
        s1 = layers.Conv3D(f0, 1, activation=None, padding='same', name=scope + "_skip_conv_1", use_bias=False,
                           kernel_initializer='he_uniform')(x)
        s1 = layers.BatchNormalization()(s1)

        c1 = layers.Conv3D(f1, (1, 3, 3), padding='same', name=scope + "_conv_1", use_bias=False,
                           kernel_initializer='he_uniform')(x)
        c1 = layers.BatchNormalization()(c1)
        c1 = layers.Activation('relu')(c1)

        c2 = layers.Conv3D(f2, (1, 3, 3), padding='same', name=scope + "_conv_2", use_bias=False,
                           kernel_initializer='he_uniform')(c1)
        c2 = layers.BatchNormalization()(c2)
        c2 = layers.Activation('relu')(c2)

        c3 = layers.Conv3D(f3, (1, 3, 3), padding='same', name=scope + "_conv_3", use_bias=False,
                           kernel_initializer='he_uniform')(c2)
