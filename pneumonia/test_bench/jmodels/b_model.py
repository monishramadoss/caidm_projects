...
xray_dataset = {'paths': datasets.download(name='ct/pna'), 
        'configs': {'batch': {'size': p['batch_size'], 'fold': p['fold']}}
}

MODEL_NAME = '{}/ckp/model.hdf5'.format(p['output_dir'])
xray_dataset['path'] = '{}/data/ymls/client-pub.yml'.format(xray_dataset['paths']['code'])
xray_dataset['client'] = Client(xray_dataset['path'], xray_dataset['configs'])
xray_dataset['gen_train'], xray_dataset['gen_valid'] = xray_dataset['client'].create_generators()
xray_dataset['inputs' = xray_dataset['client'].get_inputs(Input)

xray_dataset['log_dir'] = "{}/logs/".format(p['output_dir']) + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
if (not os.path.isdir(xray_dataset['log_dir'])):
    os.makedirs(xray_dataset['log_dir'])
